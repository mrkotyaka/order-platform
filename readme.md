...
## 📝 План для реализации (идеи, мысли, backlog)
### release v1.0
- ✔️ common-libs
- ✔️ order-service
- ✔️ payment-service
...
- ✔️ Сервис самого магазина-накопителя актуальных цен и ассортимента из магазинов с остатками товаров и пополнения. Отдельный запуск.
- Получение цены и остатков из стора в ордер сервис
- Не все товары есть в наличии. Не все количество есть в наличие
- Разные магазины для доставки. Разные цены на одни и те же позиции. Выбор магазина в ордере
- Проверка, резервирование, списание из стока при оплате, и возврат в сток при отмене
- Симуляция выбора заказа, когда нельзя отменить, новый статус?
- Сервис для учета выручки сервиса доставки (10% от стоимости товаров в store-service) 
      - ✔️ кривая логика цен
    - ✔️ отдельные dto для внутренностей стора и продукт
